...
export const CardBlog = ({ item, sx, onClick }: CardType) => {
    ...
    return (
        <div className={`w-max cardIn  relative select-none overflow-hidden h-64 lg:h-96  ${sx ? sx : ""}`} onMouseUp={() => { if(onClick){onClick()} }}>
            ...
            <div className={`w-full h-full flex flex-col justify-end p-4`}>
                <div className={`w-full overflow-hidden text-center relative border rounded transition-all duration-500 bg-bglight ${_onMouse ? "-translate-y-2 shadow-lg" : ""}`} >
