...
export const InputTable = (pros: PropsTable) => {
    ...
    // const [_table, set_table] = useState<{title:string,content:string}[]>([])
    ...
    useEffect(() => {
        ...
        if (!_onMouseDown) { saveTable(draggedItem, _indexForward); setRefresh_table(n => n + 1) }
                    ...
                    .map((tbl, index: number) =>
                        _edit ?
                            <div className='flex relative' key={index}>
                                <div className={`flex gap-4 w-full ${_onMouseDown && _index === index ? "absolute z-[0] shadow-md rounded" : "relative z-[1]"}`}
                                    ...
                                    <div className='w-full'
                                        ...
                                        onMouseUp={(e) => e.stopPropagation()}
                                        onMouseMove={(e) => e.stopPropagation()}>
                                        <TextArea sx='w-full' name="detail" onChange={(v) => { set_content(v) }} value={_index === index ? _content || tbl.content : tbl.content} onFocus={() => set_index(index)} />
                                    </div>
                                    {_index === index ?
                            ...
                            <div className='flex gap-4 w-full' key={index}>
                                <Divider name={tbl.title} sx='w-full border border-lv-4 dark:border-lv-17 line-clamp-1 ' onClick={() => set_edit(true)} />
